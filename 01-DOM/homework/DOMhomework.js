...
document.querySelector('#createdBy').innerHTML +=' Gabriela';
...
function buildToDo(todo, index) { //todo es una instancia de la clase toDo, va a tener :
  //propiedades:
  //description y complete
  // Tu código acá:
  // 1)
...
function buildToDos(toDos) {
  ...
  array = toDos.map(function(currentValue, index ){
    return buildToDo(currentValue,index)
  });
  //es lo mismo que
  // array = toDos.map(buildToDo)
  return array;
/* 
arr =  [  <div class='toDoShell'><span id=0>Comprar tomate</span></div>,
          <div class='toDoShell'><span id=1>Comprar lechuga</span></div>,
          <div class='toDoShell'><span id=2>Comprar cebolla</span></div>]
*/
...
// Tu código acá:
// 1)
var botonAgregar = document.getElementById('addButton');
botonAgregar.addEventListener('click', addToDo);
